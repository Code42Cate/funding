              ...
              <nav className="flex-1 space-y-1 px-1">
                <Link
                  ...
                  className={classNames(
                    'mb-6 flex items-center rounded-md p-2 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500'
                  )}
                >
                  <Image
                    priority
                    ...
                    width={150}
                    height={50}
                    className="mx-auto h-8 w-auto flex-shrink-0"
                  />
                </Link>
                ...
                {navigation.map((item) => (
                  <div key={item.name} className="group relative" data-cy="nav-item">
                    ...
                    {/* Help thingy */}
                    <div className="invisible absolute left-16 top-3 z-20 w-max rounded border bg-gray-50 px-2 group-hover:visible">
                      <span className="w-full text-xs text-gray-600">
                        {item.name} {item.shortcut && `(${item.shortcut.toUpperCase()})`}
                      </span>
                      <div className="absolute -left-[6px] top-[6px] border-y-[6px] border-l-0 border-r-[6px] border-solid border-y-transparent border-r-gray-200"></div>
                    </div>
                  </div>
                ))}
              </nav>
